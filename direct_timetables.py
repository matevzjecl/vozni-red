#!/usr/bin/env python3
# direct_timetables.py
#
# Usage:
#   python direct_timetables.py 1127034 1127033 1131995 > out.json
#
# Or (fallback if no args):
#   python direct_timetables.py > out.json
#   (uses all stop_ids from type_mappings.txt if present)
#
# Assumes GTFS files are in the same directory as this script:
#   agency.txt, routes.txt, trips.txt, stops.txt, stop_times.txt, calendar.txt, calendar_dates.txt
#
# Optional (auto-used if present):
#   type_mappings.txt  (stop_id,type) where type 0=bus, 1=train
#
# Output: JSON dict with only direct (consecutive) legs within a trip.
# Each entry includes "dates": list of all dates that trip runs.
...
# Force UTF-8 output (prevents "�alec" in JSON keys)
...
class Occ:
    stop_id: str
    station: str
    stop_type: Optional[int]  # 0 bus, 1 train, None unknown
    seq: int
    arr: str
    dep: str
...
def main():
    ...
    if cli_stop_ids:
        ...
        if not type_map:
            ...
            print("Usage: python direct_timetables.py <stop_id1> <stop_id2> ... > out.json", file=sys.stderr)
            sys.exit(2)
        stop_ids = list(type_map.keys())

    if len(stop_ids) < 2:
        print("Need at least 2 stop_ids (from args or type_mappings.txt).", file=sys.stderr)
        sys.exit(2)
    ...
    for trip_id, occs in trip_occs.items():
        ...
        for i in range(len(seq) - 1):
            ...
            # If both ends have known types (from type_mappings), enforce same mode and matching route_type
